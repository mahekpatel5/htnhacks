...
function YearsMultiselect({
  ...
  function toggle(year: number) {
    if (selectedYears.has(year)) {
      onUpdate({
        ...sponsor,
        years: sponsor.years.filter((y) => y.year !== year),
      });
    } else {
      onUpdate({
        ...sponsor,
...
export function SponsorsTable({
  ...
  const filtered = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      } else if (sortKey === "lastBumpDate") {
        av = new Date(a.lastBumpDate).getTime();
        bv = new Date(b.lastBumpDate).getTime();
      } else if (sortKey === "currentDri") {
        av = a.currentDri;
        bv = b.currentDri;
  ...
  function SortHeader({ label, col }: { label: string; col: SortKey }) {
    return (
      <button
        ...
        className="flex items-center justify-start gap-1 w-full text-xs font-semibold text-gray-500 uppercase tracking-wide hover:text-gray-800 transition-colors"
      >
        {label}
        {sortKey === col ? (
          <ChevronDown
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-6 py-3 border-b border-gray-100 bg-white shrink-0">
        ...
        <select
          value={filterStatus}
          onChange={(e) =>
            setFilterStatus(e.target.value as SponsorStatus | "")
          }
        ...
        <table className="w-full text-sm min-w-[960px]">
          <thead className="sticky top-0 bg-gray-50 z-10">
            <tr className="border-b border-gray-200">
              ...
              <th className="text-left px-4 py-3">
                <SortHeader label="DRI" col="currentDri" />
              </th>
              <th className="text-left px-4 py-3">
                <SortHeader label="Years" col="years" />
              </th>
              ...
              <th className="text-left px-4 py-3">
                <SortHeader label="Last Contact" col="lastBumpDate" />
              </th>
              <th className="text-left px-4 py-3 hidden lg:table-cell">
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
                  History
          ...
          <tbody>
            {filtered.map((s) => {
              ...
              const currentYearRecord = s.years.find(
                  ...
                  >
                    {currentYearRecord ? (
                      <CellDropdown<Tier>
                        ...
                        options={TIER_ORDER}
                        onSelect={(tier) => {
                          const newYears = s.years.map((y) =>
                            y.year === CURRENT_YEAR ? { ...y, tier } : y,
                          );
